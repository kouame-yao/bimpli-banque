  ...
  return (
    <div>
      <div className="mx-4 ">
        ...
        return (
        <Link  href={items.href} key={i} className={`flex-col items-center place-items-center font-semibold ${isactive ?`${color.nav}  active:scale-90 z-50 `:''} `}>{items.icon}  {items.name} </Link>
      )})}
    </div>
      </div>
  </div>
