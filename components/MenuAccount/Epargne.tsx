    ...
    <main className="flex flex-col gap-4">
      <div className={`${color.divcard} rounded-md p-2 py-4 grid gap-2`}>
        <div className="flex justify-between">
          <span>Plan Epargne Logement (PEL)</span>
          <span className="text-green-500">{dataAccount[1]?.amount} €</span>
        </div>
        <div className="h-1 bg-green-800"></div>
        <span className="grid text-right">Plafond 1 200 000.00 €</span>
      </div>

      <div className="flex flex-col gap-2">
        {dataAccount
          ?.filter(
            (row) => row.nom === "Compte épargne" || row.nom === "Livret A",
          )
          .map((items, i) => {
            return (
