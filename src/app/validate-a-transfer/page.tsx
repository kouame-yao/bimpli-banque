...
import { useGetAccounts, useGetBeneficiary, usePostransactions, } from "../../../services/lib/Requtes";
...
const odldbalance = dataAccount.find(items => items.id === input.id)
  ...
  const AddTrans = async () => {
    const now = new Date();
const formattedDate = now.toLocaleString("fr-FR", { day: "2-digit", month: "long", year: "numeric", hour: "2-digit", minute: "2-digit" });
type beneficiary = {
  Iban: string;
  Code_bic: string;
  Banque: string;
...
function validateInput() {
  ...
  const validations = [
    ...
    {
      condition: !input.id || !balance || !amount || !input.MotifP || !formattedDate || !titulaire,
...
if (!validateInput()) return;
  ...
  return (
    <div>
      {message && (
            <ErreurMessage message="Veillez remplir tout les champs"/>
      )}
      {messageSolde && (
      ...
      <main className="m-6 grid gap-4">
        ...
        <section>
          ...
          <div className="flex flex-col justify-between items-center gap-2 mt-6 mb-4">
            <select value={input.id} onChange={(e) => handchange('id', e.target.value)} className="p-2 bg-white w-full rounded-md border border-gray-500 outline-none appearance-none" name="" id="">
              <option value="">Selec... Solde</option>
              {dataAccount?.map((items, i) => {

                return (
                <option key={i} value={items?.id}>{items.nom} : {items?.amount} €  </option>
              )})}
            </select>

             <select value={input.idTitulaire} onChange={(e)=>handchange('idTitulaire',e.target.value)} className="p-2 bg-white w-full rounded-md border border-gray-500 outline-none appearance-none" name="" id="">
              <option value="">Selec... Bénéficiaire</option>
              {databenef.map((items, i) => {
                return (
                <option key={i} value={items.id}>{items.Intitule} </option>
              )})}
            </select>
          </div>
          ...
          <div className="grid gap-3">
            <div className="grid gap-3" >
                  ...
                  <input value={input.amount} onChange={(e)=>handchange('amount',e.target.value)} type="number" className="p-2 bg-white shadow-[0_0_4px_rgba(0,0,0,0.3)] rounded-md outline-none" />
            </div>
            <div className="grid gap-3" >
                  <span  className="grid place-items-center"> Motif principal </span>
                  <input value={input.MotifP} onChange={(e)=>handchange('MotifP',e.target.value)} type="text" className="p-2 bg-white shadow-[0_0_4px_rgba(0,0,0,0.3)] rounded-md outline-none" />
            </div>
            <div className="grid gap-3" >
                  <span className="grid place-items-center"> Motif complementaire (facultatif)</span>
                  <input value={input.MotifC} onChange={(e)=>handchange('MotifC',e.target.value)} type="text" className="p-2 bg-white shadow-[0_0_4px_rgba(0,0,0,0.3)] rounded-md outline-none" />
              </div>
          </div>
        </section>
      </main>
      <div className="fixed bottom-5 left-0 w-full">
        <button onClick={()=>AddTrans()} className={`${color.btn} w-full p-2 text-white text-md active:scale-90`}>
          Valider
        </button>
      </div>
      {loading && (
