    ...
    <Wrapper>
      <main className="my-6 flex flex-col gap-4 ">
        <div className="flex justify-between items-center">
          <div className="flex flex-col">
            <span className="font-bold">
              {active === "Compte" && `${resultCompte.toLocaleString()} €`}
              {active === "Epargne" && `${resultEpargne.toLocaleString()} €`}
              {active === "Crédits" && `${resultCredits.toLocaleString()} €`}
            </span>
            <span>
              {active === "Compte" && `Total compte`}
              {active === "Epargne" && `Total épargne`}
              {active === "Crédits" && `Total restant dû`}
            </span>
          </div>
          <HandCoins />
        </div>
          ...
          {MenuAccount.map((items, i) => {
            ...
            return (
              <div
                ...
                className={` px-3 py-2 rounded-full ${isActive ? `active:scale-90 ${color.btn} ` : "bg-gray-400"}`}
                key={i}
              >
                {items.name}
              </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          {active === "Compte" && <Compte />}
          {active === "Epargne" && <Epargne />}
          {active === "Crédits" && <Credit />}
          {active === "Assurance" && <Assurance />}
        </div>
      </main>
    </Wrapper>
  );
