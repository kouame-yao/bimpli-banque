  ...
  return (
    <div>
      ...
      <main className="m-6 grid gap-4">
        ...
        <section>
          ...
          <div className="grid gap-3">
            {(Object.keys(input) as (keyof beneficiary)[]).slice(0,4).map((key) => (
              <div className="grid gap-3" key={key}>
                <span className="grid place-items-center">{key.toUpperCase()}</span>
                <input
                  onChange={(e) => {
      ...
      <div className="fixed bottom-5 left-0 w-full">
        <button
          onClick={() => add()}
          className={`${color.btn} w-full p-2 text-white text-md active:scale-90`}
        >
          Ajouter
        </button>
      </div>
